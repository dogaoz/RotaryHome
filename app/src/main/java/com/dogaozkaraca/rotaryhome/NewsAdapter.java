...
public class NewsAdapter extends BaseAdapter {
    ...
    float scale;
    public NewsAdapter(Context ct, List<DoFeed_Item> feedItemList) {
        mContext = ct;
        ...
        else if(article.getType().equals("rss"))
        {
            ...
            try
            {
                if(article.getImageURL().equals("no_image_fix"))
                {
                    ...
                    String imageURL = "http://rotary.dogaozkaraca.com/default_feed_images/rotary_default_"+n+".jpg";
                    Picasso.with(mContext).load(imageURL).into(holder.ItemImage);
                }
                else
                {
...
//        int totalHeight =  holder.scrollView.getChildAt(0).getHeight();
      //  if (totalHeight > RotaryView.convertDpToPixel(48)) {
      //      holder.scrollView.clearAnimation();
      //      ObjectAnimator animScrollToBottom = ObjectAnimator.ofInt(holder.scrollView, "scrollY", holder.scrollView.getBottom());
      //      animScrollToBottom.setDuration(10000);
